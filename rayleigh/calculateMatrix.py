...
#                 left += (
#                     beam.shape(n, constraints[row_indx][0])
...
#                     right += (
#                         plate.shape(rx, ry, constraints[row_indx])
#                         * plate.shape(rx, ry, constraints[col_indx])
#                     ) / (
#                         plate.gen_mass
#                         * ((-(freq**2)) + (plate.freqs[rx - 1, ry - 1]) ** 2)
#                     )
...
#     beam_freq_diff_sq = beam.gen_mass * (
#         (-(freq**2)) + beam.freqs**2
#     )  # This is a vector for each frequency
...
#     beam_freq_diff_sq = 1 / (
#         beam.gen_mass * ((-(freq**2)) + beam.freqs**2)
#     )  # This is a vector for each frequency
...
#                 left += (
#                     beam.constraint_eval[n - 1][row_indx]
...
#                     right += (
#                         plate.constraint_eval[rx - 1, ry - 1][row_indx]
...
def construct_matrix_maybefast_tiny(freq, dim, beam, plate, debug=False):
    ...
    beam_freq_diff_sq = 1 / (
        beam.gen_mass_vector_beam * ((-(freq**2)) + beam.freqs_sq)
    )  # This is a vector for each frequency

    plate_freq_diff_sq = 1 / (plate.gen_mass_vector_plate * ((-(freq**2)) + plate.freqs_sq))
...
#     beam_freq_diff_sq = 1 / (
#         (beam.gen_mass * (-(freq**2)) + beam.freqs_sq_mass)
#     )  # This is a vector for each frequency
#     plate_freq_diff_sq = 1 / (
#         (plate.gen_mass * (-(freq**2)) + plate.freqs_sq_mass)
#     )
...
#             left = (
#                 beam.constraint_eval[:, row_indx]
#                 * beam.constraint_eval[:, col_indx]
#             )
...
#             right = (
#                 plate.constraint_eval[:, :, row_indx]
#                 * plate.constraint_eval[:, :, col_indx]
#             )
...
#             left = np.sum(
#                 np.multiply(
#                     beam.constraint_block[:, row_indx, col_indx], beam_denom
#                 )
#             )
...
#             left = np.einsum(
#                 "i,i->",
#                 beam.constraint_block[:, row_indx, col_indx],
#                 beam_denom,
#             )
