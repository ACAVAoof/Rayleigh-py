...
beam1 = Beam(np.sqrt(2), .001*70E9, 2700, .01, .01, 'PP', 15)
plate1 = Plate(np.sqrt(2), 2*np.sqrt(2), 70E9, 2700, .01, .33, 'PPPP', 15, 15)
...
constraints = [[np.pi/4, np.pi/2], [np.pi/5, np.pi/2], [2*np.pi/5, np.pi/2], [np.pi/3, np.pi/2], [np.pi/6, np.pi/2]]
#constraints = [[0.4, 0.7], [0.3, 0.7]]
#constraints = [[0.4, 0.7], [0.3, 0.7 ]]
#constraints = [[0.4, 0.4], [0.3, 0.4], [0.2, 0.4]]
#beam1.constraint_block = beam1.make_constraint_block(constraints)
...
while flag == True:
    ...
    if raw == 'Q':
        ...
        lower = float(input('Lower'))
        upper = float(input('Upper'))
        root = bisection_method(lower, upper, 50, 1E-13, beam1, plate1, dim, transform = True)
