...
def rayleigh_solve(beam, plate, constraints, freq_range, eps, loops, stopping_eps, transform = False):
    ...
    for indx in range( len (constraints) ):
        ...
        for indy in range(len( mod_freqs ) - 1):
            ...
            if len(roots) >= base_root_num and indx != 0: ##if we have found enough roots, break out
                break
            
            if mod_freqs[indy+1][0] > freq_range: ##if we are outside of the specified freq_range
                break
                        
            beam.constraint_eval = beam.constraint_shapes(constraints[0:indx+1])
            plate.constraint_eval = plate.constraint_shapes(constraints[0:indx+1])

            #print(f'On s = {indx} case')
            root = bisection_method( mod_freqs[indy][1], mod_freqs[indy+1][0], loops, stopping_eps, beam, plate, dim, transform )
        ...
        if indx == 0: ##on the first loop, we know there will be a maximum of this many roots in the range
            base_root_num = len(roots)

        
        mod_freqs = mod_freqs + mod_freqs_temp
        mod_freqs.sort(key = lambda x: x[0]) ##funky call to sort because this is list of lists
...
def asym_solve(beam, plate, constraints, freq_range, eps, loops, stopping_eps, transform = False):
...
def bisection_method( lower_bound, upper_bound, loops, stopping_eps, beam, plate, dim, transform):

    
    
    #print(f'Performing Bisection method between {lower_bound} and {upper_bound}')
    
    lower_det = np.linalg.det( construct_matrix_maybefast_tiny(lower_bound, dim, beam, plate) )
    upper_det = np.linalg.det( construct_matrix_maybefast_tiny(upper_bound, dim, beam, plate) )

    #print(f'Lower Det was {lower_det} and upper det was {upper_det}')

    C = (lower_det + upper_det)/2 

    if ( lower_det > 0 and upper_det > 0 ) or ( lower_det < 0 and upper_det < 0 ): ##same polarity
    ...
    while num_loops < loops:
        ...
        middle_det = np.linalg.det( construct_matrix_maybefast_tiny(middle, dim, beam, plate) )
        
        if transform == True:
            middle_det = sym_transform( middle_det, C )
        residual = abs(middle_det)

        if residual < stopping_eps:## if we are withing the stopping criterion
